...
public class HoldemGame {
	...
	public String toString() {
		...
		if (this.riverCard == null) {
			s += "\n\t -> probas=" + Arrays.toString(probs);  
		}
		else {
			s += "\n\t -> the Winner is: " +  
					(this.probs[0] == 100 ? "PLAYER1" : "PLAYER2") + 
					" by: " + this.bestScore[0] + " (P1) against: " + this.bestScore[1] + " (P2)";
		}
		s += "]";
		return s;
	}
...
//			}
	...
	private static void create() {	// 52 cards to create once 1->2Spade, 2->3Spade... 52->AceClub
		if (cards != null)	// do it once!
			return;
		int i=1;
		cards = new HashMap<Integer, Card>();
		for(CardColor c:CardColor.values()) {
			for (CardValue v:CardValue.values()) {
				cards.put(i++, new Card(c, v));
			}
		}
	}
